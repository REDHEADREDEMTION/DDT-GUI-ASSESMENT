...
def go_home():
    subprocess.Popen([sys.executable, "SkeletonV1/Gui.py.py"])
    root.destroy()


img = Image.open("SkeletonV1/Images/Logo.png")
...
def job_selected(event):
    ...
    if selected_job == "Engineer":
        subprocess.Popen(["python", "SkeletonV1/Jobs/Engineering.py"])
        root.destroy()

    elif selected_job == "physicest":
        subprocess.Popen(["python", "SkeletonV1/Jobs/physicest.py"])
        root.destroy()

    elif selected_job == "electrical engineer":
        subprocess.Popen(["python", "SkeletonV1/Jobs/electrical_engineer.py"])
        root.destroy()

    elif selected_job == "cook":
        subprocess.Popen(["python", "SkeletonV1/Jobs/cook.py"])
        root.destroy()

    elif selected_job == "macdonalds worker":
        subprocess.Popen(["python", "SkeletonV1/Jobs/macdonalds_worker.py"])
        root.destroy()
...
tk.Label(titleframe, text="Electrical Engineer", font=("Ariel", 40, "underline")).grid(column=0, row=0, rowspan=3,)
